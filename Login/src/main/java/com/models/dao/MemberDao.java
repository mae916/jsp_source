...
public class MemberDao {
	 ...
	 * @param request
	 */
	public static void init(ServletRequest request) { // 사이트 전역에 유지시키는 메소드
		/**
		...
		if (request instanceof HttpServletRequest) { // 안전하게 형변환(맞는지) 해줘야하기때문에 request가 HttpServletRequest가 맞는지 확인
			HttpServletRequest req = (HttpServletRequest)request; // 하위 자료형(HttpServletRequest)으로 바꿔야하기 때문에 명시적으로 형변환
			HttpSession session = req.getSession();// getSession써주어야 HttpSession 인스턴스가 반환됨
			
			int memNo = 0;
			if (session.getAttribute("memNo") != null) { // HttpSession의 반환값은 object이기 때문에 // 참조자료형의 기본값은 null
				memNo = (Integer)session.getAttribute("memNo"); // 언박싱 Integer로 형변환 하면 intvalue? 로도 가능?
			}
			
			if (memNo > 0) { // 로그인 상태
				MemberDao dao = new MemberDao();
				member = dao.get(memNo); //get() 필요한 정보를 가져오는 메서드 였음
				if (member != null) {
					isLogin = true;
				}
			} // endif 
		...
		request.setAttribute("member", member); // 이렇게 넣어주면 전역 유지가 됨.
		request.setAttribute("isLogin", isLogin); // 이렇게 넣어주면 전역 유지가 됨.
	}
	...
	public boolean join(HttpServletRequest request) throws AlertException {
		...
		try (Connection conn = DB.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) { //Statement.RETURN_GENERATED_KEYS sql의 자동증감 번호를 가져옴
			String memId = request.getParameter("memId");
			String memPw = request.getParameter("memPw");
			String memNm = request.getParameter("memNm");
			
			String hash = BCrypt.hashpw(memPw, BCrypt.gensalt(10)); // gensalt() 안에 몇번 반복 할건지
	...
	public void checkJoinData(HttpServletRequest request) throws AlertException {
		...
		for (String s : required) {
			...
			if (request.getParameter(re[0]) == null || request.getParameter(re[0]).trim().equals("")) { // 필수 항목 누락일때 
				//null값과 그냥 빈 값 인지 확인 빈값도 값이기 때문에
		...
		/** 비밀번호 확인시 일치 여부 S */
		String memPwRe = request.getParameter("memPwRe"); //getParameter -  3번째 줄 post(or get) 방식으로 넘어온 값을 가져오는것.
	...
	public void login(HttpServletRequest request, String memId, String memPw) throws AlertException {
		 ...
		 */
		 /** 필수 항목 체크 S */
		if (memId == null || memId.trim().equals("")) { //trim() 공백 제거(사용자가 아이디나 비번 입력할때 공백 넣을수도 있으니까
		...
		/** 로그인 처리 S */
		HttpSession session = request.getSession(); // getSession로 session을 불러옴
	 ...
	 * @return
	 */
	public Member get(int memNo) { // memNo(회원번호)로 DB 가지고 옴
		Member member = null;
		...
		try (Connection conn = DB.getConnection();
			...
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
				member = new Member(rs); // Member 클래스에서 rs에 다 담아줬기때문에 rs를 담은 member 생성자를 만들어줌.
			}
		} catch (SQLException | ClassNotFoundException e) {
			e.printStackTrace();
		}
